...
def download_rulekit_jar():
    ...
    current_rulekit_jars_files: List[str] = glob.glob(f"{dir_path}/jar/*-all.jar")
    ...
    asset = list(filter(lambda asset: asset['name'].endswith('-all.jar') , assets))
    if len(asset) != 1:
    ...
    if len(current_rulekit_jars_files) > 0:
        old_files_names = list(map(lambda path: os.path.basename(path), current_rulekit_jars_files))
        tmp = input(f'Old RuleKit jar file/files ({old_files_names}) detected, do you want to remove it/them? Type "yes" or "no"\n')
        if tmp == 'yes':
    ...
    urllib.request.urlretrieve(download_link, f'{dir_path}/jar/{asset["name"]}', show_progress)
